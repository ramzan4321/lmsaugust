...
class UserRegisterForm(UserCreationForm):
    ...
    def clean(self):
        ...
        # if password1 == None or not (password1.isalnum() and any(c.isalpha() for c in password1) and any(c.isdigit() for c in password1) and not password1.isalnum()):
        #     self._errors['password1'] = self.error_class(['Password must contain the combination of letter, number and special character.'])
        if len(username) < 8:
            self._errors['username'] = self.error_class(['A minimum of 8 characters is required'])
